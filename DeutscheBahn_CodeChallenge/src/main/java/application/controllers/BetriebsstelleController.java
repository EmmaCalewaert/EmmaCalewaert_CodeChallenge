...
public class BetriebsstelleController {
    ...
    ResponseEntity<String> getBetriebsstelle(@PathVariable(value = "abbreviation") String abbreviation) {
        try {
            return new ResponseEntity<>(betriebsstelleDAO.getBetriebsstelleByAbbreviationAsJSON(abbreviation), HttpStatus.OK);
        } catch (BetriebsstelleNotFoundException e) {
            return new ResponseEntity<>(String.format("No Betriebsstelle %s found", abbreviation), HttpStatus.NOT_FOUND);
        }
    }
}
